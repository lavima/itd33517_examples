...
# A function for calculating the center-aligned distance. This function can be
...
for i in range(padded_height):
  for j in range(padded_width):
    filt[i,j] = numpy.exp(-(dist2(i,j)/(2*cutoff**2)))

# Save filter for debug purposes
io.imsave("gaussian_low_pass_filter.png", filt)
...
io.imsave(sys.argv[3],numpy.clip(filtered_cropped,0,1))
